...
class comandosNotion:
    ...
    def ConsultarPagina(self, pagina_id):
    ...
    def ConsultarProyecto(self,texto):
        ...
        valores_extraidos = prompt_extract |self.model| StrOutputParser()
        ...
        if respuesta.status_code == 200:
            ...
            chain_interpretar = prompt_interpretar |self.model| StrOutputParser()
            interpretacion = chain_interpretar.invoke({"text": datos})

            return interpretacion
        else:
            return f"Error {respuesta.status_code}: No se pudo obtener los datos"
    ...
    def ConsultarTareas(self,texto):
        # Define el prompt para extraer valores del texto de entrada
        prompt_extract = ChatPromptTemplate.from_template(
            """
        ...
        if respuesta.status_code == 200:
            ...
            speech = f"Error {respuesta.status_code}: No se pudo obtener los datos"
            return speech
    ...
    # Función principal para consultar y procesar tareas
    def ConsultarSprint(self,texto):
        # Define el prompt para extraer valores del texto de entrada
        prompt_extract = ChatPromptTemplate.from_template(
            """
    ...
    # Función principal para consultar y procesar tareas
    def ConsultarMinuta(self,texto):
        # Define el prompt para extraer valores del texto de entrada
        prompt_extract = ChatPromptTemplate.from_template(
            """
            Extrae el nombre del Sprint del comando:
    ...
    def EliminarProyecto(self,texto):
        # Define el prompt para extraer valores del texto de entrada
        prompt_extract = ChatPromptTemplate.from_template(
            """
            Extrae el nombre del proyecto del comando:
        ...
        if respuesta.status_code != 200:
            return f"Error {respuesta.status_code}: No se pudo obtener los datos del proyecto."

        resultados = respuesta.json().get("results", [])
        if not resultados:
            return "No se encontró ningún proyecto con ese nombre."
        ...
        if respuesta_archivar.status_code == 200:
            return "Proyecto eliminado (archivado) con éxito."
        else:
            return f"Error {respuesta_archivar.status_code}: No se pudo archivar el proyecto."
    ...
    def EliminarTarea(self,texto):
        ...
        respuesta = requests.post(url_query, headers=cabecera, data=json.dumps(filtro))
        if respuesta.status_code != 200:
            return f"Error {respuesta.status_code}: No se pudo obtener los datos de la tarea."

        resultados = respuesta.json().get("results", [])
        if not resultados:
            return "No se encontró ninguna tarea con ese nombre."
        ...
        respuesta_archivar = requests.patch(url_update, headers=cabecera, data=json.dumps(cuerpo))
        if respuesta_archivar.status_code == 200:
            return "Tarea eliminada (archivada) con éxito."
        else:
            return f"Error {respuesta_archivar.status_code}: No se pudo archivar la tarea."
    ...
    def EliminarSprint(self,texto):
        ...
        respuesta = requests.post(url_query, headers=cabecera, data=json.dumps(filtro))
        if respuesta.status_code != 200:
            return f"Error {respuesta.status_code}: No se pudo obtener los datos del sprint."
            

        resultados = respuesta.json().get("results", [])
        if not resultados:
            return "No se encontró ningún sprint con ese nombre."
        ...
        respuesta_archivar = requests.patch(url_update, headers=cabecera, data=json.dumps(cuerpo))
        if respuesta_archivar.status_code == 200:
            return "Sprint eliminado (archivado) con éxito."
        else:
            return f"Error {respuesta_archivar.status_code}: No se pudo archivar el sprint."
    ...
    def EliminarMinuta(self,texto):
        ...
        respuesta = requests.post(url_query, headers=cabecera, data=json.dumps(filtro))
        if respuesta.status_code != 200:
            return f"Error {respuesta.status_code}: No se pudo obtener los datos de la minuta."

        resultados = respuesta.json().get("results", [])
        if not resultados:
            return "No se encontró ninguna minuta con ese nombre."
        ...
        respuesta_archivar = requests.patch(url_update, headers=cabecera, data=json.dumps(cuerpo))
        if respuesta_archivar.status_code == 200:
            return "Minuta eliminada (archivada) con éxito."
        else:
            return f"Error {respuesta_archivar.status_code}: No se pudo archivar la minuta."
    ...
    def ExtraerNombreTarea(self,texto):
    ...
    def BuscarTarea(self,nombre_tarea):
        filtro = {
            "filter": {
                "property": "Nombre de la tarea",
                "rich_text": {
        ...
        respuesta = requests.post(self.URL_BUSQUEDA, headers=self.CABECERA, data=json.dumps(filtro))
    ...
    def ActualizarTarea(self,page_id, texto):
        promptc = ChatPromptTemplate.from_template("""
        Extrae el nombre del proyecto, el nombre de la tarea, la fecha de inicio, la fecha de conclusión, el nivel de prioridad, el nombre
        de la persona a la que esta asignada, el estado y la descripción del siguiente comando:
        {texto}
        ...
        url_actualizacion = self.URL_ACTUALIZACION.format(page_id=page_id)
        respuesta = requests.patch(url_actualizacion, headers=self.CABECERA, data=json.dumps(actualizacion_tarea))
